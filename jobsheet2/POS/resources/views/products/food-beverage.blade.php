...
<head>
    ...
    <title>Food &amp; Beverage — PointSales</title>
...
<body>
    ...
    <aside class="sidebar">
        ...
        <div class="sidebar-section" style="margin-top:.5rem;">
            ...
            <a href="/category/food-beverage" class="nav-item active"><span class="ico">🍔</span> Food &amp;
                Beverage</a>
    ...
    <div class="topbar">
        <div class="topbar-left">
            <h2>🍔 Food &amp; Beverage</h2>
            <p>Manajemen produk kategori makanan dan minuman</p>
        </div>
        <div class="topbar-right">
            <div class="live-clock" id="clock">--:--:--</div>
            <a href="/sales" class="btn-top primary"><i class="fa-solid fa-cash-register"></i> Buka Kasir</a>
        </div>
    </div>
    ...
    <main class="main">
        <div class="content">
            ...
            <div class="page-h fade">
                <div class="page-h-left">
                    <h1>🍔 Food &amp; Beverage</h1>
                    <p>Makanan, minuman, snack, dan produk konsumsi sehari-hari tersedia di sini.</p>
                    <div class="page-h-badges">
                        <span class="badge"><span class="dot"></span> Stok Aktif</span>
                        <span class="badge"><i class="fa-solid fa-boxes-stacked"></i> 120 SKU</span>
                        <span class="badge"><i class="fa-solid fa-tag"></i> 8 Produk Ditampilkan</span>
                        <span class="badge" style="color:var(--amber);border-color:rgba(245,158,11,.3);"><i
                                class="fa-solid fa-triangle-exclamation"></i> 1 Stok Menipis</span>
                    </div>
                </div>
            ...
            <div class="toolbar fade">
                <div class="search-wrap">
                    ...
                    <input type="text" id="searchInput" placeholder="Cari produk, kode, atau barcode…"
                        onkeyup="filterProds()">
                </div>
        ...
        <footer>
            <p>© 2026 <strong style="color:var(--ts);">Athaulla Hafizh</strong> · PointSales POS System · Kategori Food
                &amp; Beverage</p>
            <a href="/" style="font-size:.72rem;color:var(--tm);" onmouseover="this.style.color='#808080'"
                onmouseout="this.style.color='#484848'">← Dashboard</a>
        </footer>
    </main>
    ...
    <script>
        const PRODS = [
            { id: 'FB001', name: 'Teh Botol Sosro', desc: '330ml · Minuman Teh', price: 5000, emoji: '🥤', stock: 85, cat: 'Minuman' },
            { id: 'FB002', name: 'Kopi Good Day', desc: '25gr · Kopi Instan', price: 3500, emoji: '☕', stock: 120, cat: 'Minuman' },
            { id: 'FB003', name: 'Indomie Goreng', desc: '85gr · Mie Instant', price: 4000, emoji: '🍜', stock: 200, cat: 'Makanan' },
            { id: 'FB004', name: 'Oreo Original', desc: '133gr · Biskuit', price: 15000, emoji: '🍪', stock: 60, cat: 'Snack' },
            { id: 'FB005', name: 'Susu Ultra Milk', desc: '200ml · Susu UHT', price: 6000, emoji: '🥛', stock: 95, cat: 'Minuman' },
            { id: 'FB006', name: 'Aqua Galon', desc: '19L · Air Mineral', price: 22000, emoji: '🌊', stock: 15, cat: 'Minuman' },
            { id: 'FB007', name: 'Chitato Original', desc: '68gr · Keripik', price: 12000, emoji: '🍿', stock: 55, cat: 'Snack' },
            { id: 'FB008', name: 'SilverQueen Almond', desc: '62gr · Cokelat', price: 18000, emoji: '🍫', stock: 30, cat: 'Snack' },
        ];
        let showLowOnly = false, sortAZ = false;
        let currentData = [...PRODS];
        const fmt = n => 'Rp ' + n.toLocaleString('id-ID');
        function render() {
            const q = document.getElementById('searchInput').value.toLowerCase();
            let data = currentData.filter(p => !q || p.name.toLowerCase().includes(q) || p.id.toLowerCase().includes(q));
            if (showLowOnly) data = data.filter(p => p.stock <= 20);
            document.getElementById('prodGrid').innerHTML = data.map(p => {
                const stockCls = p.stock === 0 ? 'stock-out' : p.stock <= 20 ? 'stock-low' : 'stock-ok';
                const stockTxt = p.stock === 0 ? 'Habis' : p.stock <= 20 ? `Sisa ${p.stock}` : `Stok ${p.stock}`;
                return `<div class="prod-card">
            <div class="prod-img"><span class="prod-cat-label">${p.cat}</span>${p.emoji}<span class="stock-pill ${stockCls}">${stockTxt}</span></div>
            <div class="prod-body">
                ...
                <div class="prod-footer">
                    ...
                    <button class="btn-add" onclick="addToCartLS('${p.id}','${p.name.replace(/'/g, \"\\'\")}',${ p.price},'${p.emoji}')" ${ p.stock === 0 ? 'disabled style="opacity:.4;cursor:not-allowed;"' : '' } > <i class="fa-solid fa-cart-plus"></i></button >
                </div >
            </div >
        </div > `;
            }).join('') || '<p style="color:var(--tm);font-size:.82rem;grid-column:1/-1;text-align:center;padding:3rem;">Produk tidak ditemukan.</p>';
        }
        ...
        function addToCartLS(id, name, price, emoji) {
            const LS_KEY = 'pos_cart';
            let cart = [];
            try { cart = JSON.parse(localStorage.getItem(LS_KEY) || '[]'); } catch(e) {}
            const idx = cart.findIndex(x => x.id === id);
            if (idx >= 0) cart[idx].qty++;
            else cart.push({ id, qty: 1 });
            localStorage.setItem(LS_KEY, JSON.stringify(cart));
            const total = cart.reduce((s, i) => s + i.qty, 0);
            showToast(`${ emoji } ${ name } ditambahkan ke kasir!(${ total } item di keranjang)`);
        }
